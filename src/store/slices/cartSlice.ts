...
//   source: string | undefined,
//   cartItem: any | null,
//   suppressAddedToast?: boolean,
// ) => {
//   if (suppressAddedToast) {
//     return;
//   }
//   if (source === 'prescribed') {
//     return;
...
// const normalizeCartItemsImages = (data: CartData): CartData => {
//   const markStock = (item: any) => {
//     const outOfStock = isCartLineOutOfStock(item);
//     return normalizeCartLineItem({
...
//   const myItems = (data?.my_cart?.items ?? []).map(markStock);
//   const prescriptionGroups = (data?.prescription_cart?.items ?? []).map(
//     (group: any) => ({
//       ...group,
//       items: (group?.items ?? []).map(markStock),
//     }),
//   );
...
//   currentQuantity?: number;
//   prescriptionRequired?: boolean;
//   /** When true, skip the global "Added to cart / View cart" overlay (e.g. MyCart). */
//   suppressAddedToast?: boolean;
// };
...
//       // Prescribed lines: same cart URL as my_cart (no source= query).
//       // Never remove (qty 0). Always send cart_item_id + absolute quantity.
//       if (source === 'prescribed') {
//         if (Boolean(prescriptionRequired)) {
//           return rejectWithValue('Prescription required');
//         }
...
//         if (!safeVariantId) {
//           return rejectWithValue('Missing variant id');
//         }
...
//           quantity: Number(prescribedCartItem?.quantity ?? nextQty),
//           cartItemId: prescribedCartItemId,
//           source: 'prescribed' as const,
//           cartItem: prescribedCartItem,
//           message:
//             prescribedResponse?.message || 'Cart quantity updated',
//         };
//       }
...
//               currentQuantity: latest.currentQuantity,
//               prescriptionRequired: latest.prescriptionRequired,
//               suppressAddedToast: Boolean(latest.suppressAddedToast),
//             }),
//           );
...
type LineSyncPayload = {
  ...
  /** When true, skip the global "Added to cart / View cart" overlay (e.g. MyCart). */
  suppressAddedToast?: boolean;
  /** Listing screens toast the rejected payload themselves. */
  ...
  async (
    ...
    try {
      ...
      if (source === 'prescribed') {
        ...
        if (prescribedResponse?.success === false) {
          return rejectWithValue(
            formatOrderStockError(prescribedResponse?.message) ||
            prescribedResponse?.message ||
            'Failed to update prescribed item',
          );
        }
      ...
      return rejectWithValue(
        formatOrderStockError(error?.message) ||
        error?.message ||
        'Failed to update cart',
      );
    }
      ...
      lineSyncTimers.set(
        ...
        setTimeout(async () => {
          ...
          try {
            ...
            if (!latest.suppressErrorToast) {
              const failMsg =
                formatOrderStockError(
