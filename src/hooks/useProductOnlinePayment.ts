...
type OnlinePaymentArgs = {
  ...
  customerInfo?: any;
  shippingFee: number;
  onSuccess: (orderResult: any, orderedCartItems: Array<{ variant_id: string; quantity: number }>) => void;
};
...
export const useProductOnlinePayment = () => {
  ...
  const payOnline = useCallback(
    async ({
      ...
      if (isPaying || paymentStartedRef.current) {
        return;
      }
      ...
      const payableAmount = Math.round(subtotal + shippingFee);
