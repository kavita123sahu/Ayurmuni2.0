...
const MyCart = ({ navigation }: any) => {
    ...
    const cartInStockCount =
        cartSection?.items.filter((item: any) => !item?._isOutOfStock).length ??
        0;
    const prescribedInStockCount =
        prescribedSection?.items.filter((item: any) => !item?._isOutOfStock)
            .length ?? 0;
    const showTabs =
        cartCount > 0 &&
        prescribedCount > 0 &&
    ...
    // OOS panel is scoped to the active tab only (My Cart vs Prescribed)
    ...
    return (
        ...
        >
            ...
            <CommonModal
                ...
                subtitle={
                    qtyConfirm
                        ? `Doctor prescribed ${qtyConfirm.doctorQty} ${qtyConfirm.doctorQty === 1 ? 'unit' : 'units'
...
//             doctorQtyByLineRef.current = {};
//             setFocusTick(tick => tick + 1);
//             // Always reload cart so stock / qty changes are reflected
//             fetchAllData({
//                 force: true,
//                 silent: true,
//             });
...
//     const outOfStockItems = useMemo(() => {
//         const sectionType = activeTab === 'cart' ? 'cart' : 'prescribed';
...
//     const cartCount = cartSection?.items.length ?? 0;
//     const prescribedCount = prescribedSection?.items.length ?? 0;
//     const showTabs = cartCount > 0 && prescribedCount > 0;
