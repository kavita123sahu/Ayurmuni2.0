...
//         refreshPreview();
//       }
//       // Guest / empty catalog: reload so products show when token is ready
//       const noProducts =
//         !(storeProducts?.length > 0) && !(medicineProducts?.length > 0);
//       if (noProducts && shouldRunThrottled('home-catalog-empty-refresh', 20_000)) {
//         refreshHomeData();
//       }
//     }, [refreshPreview, refreshHomeData, storeProducts?.length, medicineProducts?.length]),
//   );
...
//       }
//     },
//     [fetchCustomerData, refreshHomeData, refreshHealthConcerns],
//   );
...
const HomePage: React.FC = (props: any) => {
  ...
  const hasSelectedDiseases = useMemo(() => {
    const ids = Array.isArray(customerData?.health_disease_ids)
      ? customerData.health_disease_ids
      : [];
    const objects = Array.isArray(customerData?.health_diseases)
      ? customerData.health_diseases
      : [];
    return (
      customerData?.has_health_diseases === true ||
      ids.length > 0 ||
  ...
  const doctorSectionTitle = hasSelectedDiseases
    ? 'Suggested Doctors'
    : 'Meet Our Doctors';
  ...
  const productSectionTitle = hasSelectedDiseases
    ? 'Suggested Products'
    : 'Shop by product ';
  const yogaSectionTitle = hasSelectedDiseases ? "Suggested Yoga" : "Yoga for You";
  const dietSectionTitle = hasSelectedDiseases ? "Suggested Diet" : "Diet for You";
      ...
      <FlatList
        ...
        renderItem={() => (
          <View style={styles.sections}>
            ...
            {loadingDoctors && !(SuggestDoctor?.length > 0) ? (
              <View style={styles.homeSection}>
                <SectionHeader home title={doctorSectionTitle} />
                <TopDoctorsCardSkeleton count={4} featured />
              </View>
            ) : null}
            ...
            {loadingProducts && !(medicineProducts?.length > 0) ? (
              <View style={styles.homeSection}>
                <SectionHeader home title={medicineSectionTitle} />
                <TopSellingListSkeleton />
              </View>
            ) : null}
            ...
            {loadingProducts && !(storeProducts?.length > 0) ? (
              <View style={styles.homeSection}>
                <SectionHeader home title={productSectionTitle} />
                <TopSellingListSkeleton />
              </View>
            ) : null}
